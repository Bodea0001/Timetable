...
def create_application(
    ...
    application = models.Application(
        id_user=user_id, id_timetable=timetable_id)
    db.add(application)
    db.commit()
...
def have_user_application(
        db: Session, user_id: int | Column[Integer],
        application_id: int | Column[Integer]) -> bool:
...
def get_application_by_id(
        db: Session, application_id: int | Column[Integer]
...
def get_applications_by_list_with_timetables_id(
        db: Session, timetables_id: list[int] | list[Column[Integer]] | None
    ) -> list[models.Application]:
    """Ищет заявки в БД по ID расписаний, к которым
    эти заявки относятся, и отдаёт"""
    return db.query(models.Application).filter(
        models.Application.id_timetable.in_(timetables_id)).all()
