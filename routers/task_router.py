...
async def create_task_for_user(
    ...
    timetable_current_user_status = get_timetable_user_status(
        db, user.id, task.id_timetable)
    if (timetable_current_user_status != schemas.TimetableUserStatuses.elder or
        user_id != user.id):
        raise HTTPException(
...
async def get_timetable_tasks_for_elder(
    ...
    user_timetable_tasks = [validate_task_for_user(task, user.id)  # type: ignore
...
async def get_user_tasks_in_timetable(
    ...
    elder_timetable_tasks = [validate_task_for_elder(db, task)  # type: ignore 
                            for task in timetable.tasks]
...
async def get_all_user_tasks(user: models.User = Depends(get_current_user)):
    user_tasks = [validate_task_for_user(task, user.id)
        for timetable in user.timetables_info for task in timetable.tasks]  # type: ignore
...
async def filter_user_tasks_by_date(
    ...
    user_timetable_tasks = [validate_task_for_user(task, user.id) for task in tasks]
    user_timetable_sorted_tasks = [task for task in user_timetable_tasks if task]
    if user_timetable_sorted_tasks:
        return user_timetable_sorted_tasks
...
async def update_task_information(
    ...
    if task.tag == schemas.TaskTags.all:
        ...
        timetable_user_status = get_timetable_user_status(
            db, user.id, task.id_timetable)  # type: ignore
        if timetable_user_status != schemas.TimetableUserStatuses.elder:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"The current user doesn't have access rights to update this task")
    
    update_task_info(db, task_data.id, task_data.description, 
        task_data.subject, task_data.deadline)
...
async def delete_task(
    ...
    timetable_user_status = get_timetable_user_status(
        db, user.id, task.id_timetable)  # type: ignore
    user_task_relation = get_user_task_relation_by_task_id(db, task_id, user.id)
    if (task.tag == schemas.TaskTags.one and not user_task_relation or
        timetable_user_status != schemas.TimetableUserStatuses.elder):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def delete_expired_user_tasks(
    ...
    user_timetable_task_ids = get_complited_task_ids_by_user_id_timetable_id(
        db, user.id, timetable_id)
